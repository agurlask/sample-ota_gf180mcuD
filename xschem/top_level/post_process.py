...
if __name__ == '__main__':
    data = read_ngspice_raw('output.raw')
    ...
    # find frequency where mag_db crosses 0 dB
    ugbw_idx = np.argmin(np.abs(mag_db-0))
    dc_freq = 10
    dc_freq_idx = np.argmin(np.abs(freq-dc_freq))

    fig, ax1 = plt.subplots(figsize=(8, 5))

    ax1.semilogx(freq, mag_db, color='tab:blue', label='Magnitude (dB)')
    ax1.scatter(freq[ugbw_idx], mag_db[ugbw_idx], marker='o', color='tab:blue')
    ...
    ax2.scatter(freq[ugbw_idx], phase_deg[ugbw_idx], marker='o', color='tab:red')
    ax2.axvline(x=freq[ugbw_idx], linestyle='dotted', color='tab:red')
    ax2.set_ylabel('Phase (deg)', color='tab:red')
    ax2.tick_params(axis='y', labelcolor='tab:red')
    ax2.set_ylim([-80, 200])
    plt.title(f'Phase Margin={phase_deg[ugbw_idx]:.1f} deg, DC Gain={mag_db[dc_freq_idx]:.1f} dB, UGBW: {freq[ugbw_idx]*1e-6:.1f} MHz')
    fig.tight_layout()
    plt.show()

    fig.savefig('freq_resp.png')
